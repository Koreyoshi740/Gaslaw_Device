...
private:
    int32_t& _value;
    int32_t _minValue, _maxValue, _step;
    float _displayScale;
    const char* _title;
    ...
    int32_t _fastRepeatCount = 0;
public:
    PopupProgress(PixelUI& ui, uint16_t width, uint16_t height,
                  int32_t& value, int32_t minValue, int32_t maxValue,
                  const char* title = "", uint16_t duration = 3000, uint8_t priority = 0, std::function<void(int32_t value)> cb_function = nullptr, bool UseApparentVal = false, int32_t step = 1, float display_scale = 1.0f);

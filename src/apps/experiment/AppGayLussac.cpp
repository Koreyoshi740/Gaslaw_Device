...
static const unsigned char image_gaylussac_bits[] = {
  0xF0, 0xFF, 0x0F, 0xFC, 0xFF, 0x3F, 0xFE, 0x00, 0x78, 0xBE, 0xFE, 0x7B,
  0xBF, 0xFE, 0xFB, 0x0F, 0xF8, 0xF1, 0x0F, 0xF8, 0xFB, 0x1F, 0xFC, 0xF1,
  0xEF, 0xFB, 0xE0, 0xF7, 0x77, 0xC0, 0xDB, 0x6E, 0xDB, 0x9B, 0x2F, 0x91,
  0xCB, 0x28, 0x80, 0x9B, 0x2A, 0x91, 0xCB, 0x6E, 0xDB, 0x9B, 0xEA, 0xE0,
  0xDB, 0xE8, 0xF1, 0x8B, 0xAF, 0xFE, 0xAB, 0xAD, 0xFE, 0x8B, 0xA8, 0xF6,
  0xF2, 0x27, 0x62, 0x06, 0x70, 0x41, 0xFC, 0xFF, 0x3F, 0xF0, 0xFF, 0x0F
};
...
static int32_t g_targetVolume = 250;   // mL

// MANUAL mode temp steps
static int32_t g_manualSteps[GayLussacLaw::MAX_STEPS] = {};
static int32_t g_stepCount = 0;

// 压强控制
static bool    g_pressCtrl      = false;
static int32_t g_targetPressure = 1013;  // hPa
static int32_t g_initVolume     = 250;   // mL

// ===== MANUAL Step sub-menu =====
class AppGayLussacStepMenu : public ListView {
private:
    ListItem m_items[11];
    ...
    void rebuildItems() {
        ...
        if (g_stepCount < (int32_t)GayLussacLaw::MAX_STEPS) {
            setTitle(m_items[base], "添加节点");
            m_items[base].pFunc = [this]() {
                if (g_stepCount >= (int32_t)GayLussacLaw::MAX_STEPS) return;
...
public:
    AppGayLussacStepMenu(PixelUI& ui) : ListView(ui, m_items, 11) {}
...
// ===== Settings ListView =====
class AppGayLussacSettings : public ListView {
private:
    // 10 items: 标题 + 自动模式 + kPa单位 + 压强控制 + 目标压强 + 初始体积 + 温度节点 + 气阀 + 开始实验 + 实验报告
    ListItem              m_items[10];
    AppGayLussacStepMenu  m_stepMenu;
    ...
    void rebuildModeItems() {
        // 索引 4：目标压强（行为随 g_pressCtrl 变化）
        clearItem(m_items[4]);
        setTitle(m_items[4], "目标压强");
        m_items[4].extra.intValue = &g_targetPressure;
        m_items[4].pFunc = [this]() {
            if (g_pressCtrl) {
                m_ui.showPopupProgress(g_targetPressure, 900, 1100,
                    "目标压强(hPa)", 100, 40, 4000, 1, nullptr, true, 1);
            } else {
                float p = bme.getPressure();
                char buf[32];
                if (!isnan(p)) snprintf(buf, sizeof(buf), "Current: %.1f hPa", p);
                else           snprintf(buf, sizeof(buf), "Current: ---.- hPa");
                m_ui.showPopupInfo(buf, "默认恒压目标", 110, 40, 2500);
            }
        };

        // 索引 6：温度节点（行为随 g_autoMode 变化）
        clearItem(m_items[6]);
        setTitle(m_items[6], "温度节点");
        if (g_autoMode) {
            ...
            m_items[6].extra.intValue = &g_stepCount;
            m_items[6].pFunc = [this]() {
                m_inStepMenu = true;
        ...
        // 索引 7：气阀
        clearItem(m_items[7]);
        setTitle(m_items[7], "气阀");
        m_items[7].extra.switchValue = &g_valveOpen;
        m_items[7].pFunc = [this]() {
            if (g_valveOpen) gayLussac.openValve();
            else             gayLussac.closeValve();
        };

        // 索引 8：开始实验
        clearItem(m_items[8]);
        setTitle(m_items[8], "开始实验");
        ...
        // 索引 9：实验报告
        clearItem(m_items[9]);
        setTitle(m_items[9], "实验报告");
        m_items[9].pFunc = [this]() {
    ...
    void setupItems() {
        ...
        setTitle(m_items[3], "压强控制");
        m_items[3].extra.switchValue = &g_pressCtrl;
        m_items[3].pFunc = [this]() { rebuildModeItems(); };

        setTitle(m_items[5], "初始体积");
        m_items[5].extra.intValue = &g_initVolume;
        m_items[5].pFunc = [this]() {
            m_ui.showPopupProgress(g_initVolume, 100, 450,
                "初始体积(mL)", 100, 40, 4000, 1, nullptr, true, 25);
        };
...
public:
    ...
    AppGayLussacSettings(PixelUI& ui) : ListView(ui, m_items, 10), m_stepMenu(ui) {
        setupItems();
    }
    ...
    void reattachFocus() {
        // 重新进入设置页时，同步硬件气阀状态到开关显示
        g_valveOpen = gayLussac.isValveOpen();
        m_ui.markDirty();
    }

    void onLoad() override { g_valveOpen = gayLussac.isValveOpen(); }
    void onSave() override {}
    void onExit() override {}
};
...
private:
    ...
    void entryCoro(CoroutineContext& ctx) {
        CORO_BEGIN(ctx);
            ...
            m_ui.animate(anim_pres_x,   0, 350, EasingType::EASE_OUT_CUBIC, PROTECTION::PROTECTED);
    ...
    StateInfo stateInfo() {
        switch (gayLussac.getState()) {
            case GayLussacLaw::INIT_VOLUME: return {"Fill", "INITVOL", 0};
            case GayLussacLaw::PRES_INIT:   return {"PWat", "WAITPRE", 500};
            case GayLussacLaw::STEPPING:    return {"Heat", "HEATING", 500};
            case GayLussacLaw::RECORDING:   return {"Rec",  "RECORD",  150};
            case GayLussacLaw::DONE:        return {"Done", "DONE",    0};
            case GayLussacLaw::ERROR:       return {"Err",  "ERROR",   300};
    ...
    void braceContent() {
        ...
        auto drawPage = [&](BracePage page, int yBase) {
            ...
            if (page == BracePage::CURRENT) {
                ...
                u8g2.setDrawColor(1);
                GayLussacLaw::State s = gayLussac.getState();
                bool running = (s == GayLussacLaw::STEPPING || s == GayLussacLaw::RECORDING);
                float t = running ? gayLussac.getNextTargetTemp()
                                  : gayLussac.getCurrentTargetTemp();
                if (t >= 0.0f) {
    ...
    void drawStatusBar() {
        ...
        // Left: current pressure slides in
        float p = bme.getPressure();
        if (isnan(p))       snprintf(m_buf, sizeof(m_buf), "P=-- hPa");
        else if (g_unitIsKpa) snprintf(m_buf, sizeof(m_buf), "P=%.2fkPa", p / 10.0f);
        else                snprintf(m_buf, sizeof(m_buf), "P=%.0fhPa",  p);
        u8g2.drawStr(anim_pres_x, 7, m_buf);

        // Center: step progress bar
        uint8_t total = gayLussac.getTotalSteps();
        if (total > 0) {
            uint8_t cur = gayLussac.getCurrentStep();
    ...
    void drawMainArea() {
        ...
        // Main: air volume (large font)
        u8g2.setFont(u8g2_font_profont17_tr);
        float v = volumeCtrl.getAirVolume();
        if (isnan(v)) snprintf(m_buf, sizeof(m_buf), "---.-- mL");
        else          snprintf(m_buf, sizeof(m_buf), "%.1f mL", v);
...
public:
    ...
    AppGayLussacRunning(PixelUI& ui)
        : m_ui(ui)
        , m_brace(ui, 3, 45, 56, 18)
        , m_histogram(ui, 69, 45, 56, 18, 76, 63, EXPAND_BASE::BOTTOM_RIGHT, (char*)"Vol")
        , m_blinker(ui, 500)
    ...
    void draw() {
        ...
        if (now - m_prevTime >= 500) {
            m_prevTime = now;
            m_histogram.addData(volumeCtrl.getAirVolume());
        }
        ...
        if (m_histogram.isExpanded()) {
            ...
            float vmax = m_histogram.getMaxValueInHistory();
            float vavg = m_histogram.getAverageValueInHistory();

            u8g2.drawStr(0, 20, "Max:");
            snprintf(m_buf, sizeof(m_buf), "%.1f mL", vmax);
            u8g2.drawStr(0, 30, m_buf);

            u8g2.drawStr(0, 42, "Avg:");
            snprintf(m_buf, sizeof(m_buf), "%.1f mL", vavg);
...
private:
    ...
    // 快照：保存上一次实验数据，reset() 不会影响它
    GayLussacLaw::DataPoint m_snapData[GayLussacLaw::MAX_STEPS] = {};
    uint8_t                 m_snapCount   = 0;
    float                   m_snapTargetP = 0.0f;
    ...
    void drawRowAt(int rowIndex, int yBase, int xOff) {
        ...
        if (rowIndex < (int)m_snapCount) {
            ...
            int tw = u8g2.getStrWidth(m_buf);
            u8g2.drawStr(xOff + 42 - tw, yBase, m_buf);

            // T 列
            if (!m_snapTempIsK) snprintf(m_buf, sizeof(m_buf), "%.1f", dp.T - 273.15f);
            else                snprintf(m_buf, sizeof(m_buf), "%.1f", dp.T);
            tw = u8g2.getStrWidth(m_buf);
            u8g2.drawStr(xOff + 82 - tw, yBase, m_buf);

            // V/T 列
            snprintf(m_buf, sizeof(m_buf), "%.3f", dp.VT);
            tw = u8g2.getStrWidth(m_buf);
...
public:
    AppGayLussacReport(PixelUI& ui) : m_ui(ui) {}
    ...
    // 保存当前实验数据快照（在实验结束/退出时调用）
    void saveSnapshot() {
        m_snapCount   = gayLussac.getDataCount();
        m_snapTargetP = gayLussac.getTargetPressure();
        m_snapTempIsK = g_tempIsK;
        for (uint8_t i = 0; i < m_snapCount; i++)
            m_snapData[i] = gayLussac.getDataPoint(i);
    }
    ...
    void drawAt(int xOff) {
        ...
        if (m_snapCount == 0) {
            u8g2.drawStr(xOff + 20, 32, "No Data");
            return;
        }

        // 表头
        u8g2.drawStr(xOff + 0,  HEADER_Y, "#");
        u8g2.drawStr(xOff + 16, HEADER_Y, "V(mL)");
        const char* tUnit = m_snapTempIsK ? "T(K)" : "T(C)";
        u8g2.drawStr(xOff + 56, HEADER_Y, tUnit);
        u8g2.drawStr(xOff + 90, HEADER_Y, "V/T");
        ...
        if (!g_unitIsKpa) snprintf(m_buf, sizeof(m_buf), "P:%.0fhPa", m_snapTargetP);
        else              snprintf(m_buf, sizeof(m_buf), "P:%.1fkPa", m_snapTargetP / 10.0f);
...
// ===== Main App state =====
enum class GayLussacState { MAIN_IDLE, NEXT_PAGE, RUNNING, REPORT };
enum class GayLussacLoadState { INIT, BRACE_LOADING, DONE };

class AppGayLussac : public IApplication {
private:
    PixelUI&            m_ui;
    GayLussacState      state;
    GayLussacLoadState  loadState;

    AppGayLussacSettings m_settings;
    AppGayLussacRunning  m_running;
    AppGayLussacReport   m_report;

    // slideX 动画（运行页/报告页滑入）
    int32_t m_slideX    = 0;
    bool    m_sliding   = false;

    Brace      pressBrace;   // 左：压强 hPa/kPa
    Brace      tempBrace;    // 右：温度 °C/K
    ...
    // Press brace flip: hPa <-> kPa
    bool    currentPressUnit = false;  // false = hPa, true = kPa
    bool    targetPressUnit  = false;
    int32_t anim_press_y     = 0;
    bool    press_animating  = false;
    ...
    void pressBraceContent() {
        U8G2& u8g2 = m_ui.getU8G2();
        u8g2.setFont(u8g2_font_5x7_tr);
        const int PAGE_HEIGHT = 18;
        float p = bme.getPressure();

        auto drawPage = [&](bool kpa, int y_base) {
            if (y_base < 45 || y_base > 70) return;
            u8g2.drawRBox(8, y_base - 8, 24, 10, 2);
            u8g2.setDrawColor(0);
            u8g2.drawStr(10, y_base, "Pres");
            u8g2.setDrawColor(1);
            char buf[16];
            if (kpa) snprintf(buf, sizeof(buf), "%.2f", isnan(p) ? 0.0f : p / 10.0f);
            else     snprintf(buf, sizeof(buf), "%.1f", isnan(p) ? 0.0f : p);
            int tw = u8g2.getStrWidth(buf);
            u8g2.drawStr(58 - tw, y_base - 4, buf);
            u8g2.drawStr(40, y_base + 3, kpa ? "kPa" : "hPa");
        };

        int cy = 58 + anim_press_y;
        int ny = 58 + anim_press_y - PAGE_HEIGHT;
        drawPage(currentPressUnit, cy);
        drawPage(targetPressUnit,  ny);
    }
    ...
    void tempBraceContent() {
        ...
        auto drawPage = [&](bool celsius, int y_base) {
            ...
            u8g2.setDrawColor(1);
            char buf[16];
            if (celsius) snprintf(buf, sizeof(buf), "%.1f", isnan(temp) ? 0.0f : temp);
            else         snprintf(buf, sizeof(buf), "%.1f", isnan(temp) ? 0.0f : temp + 273.15f);
            int tw = u8g2.getStrWidth(buf);
    ...
    void drawMainIdle() {
        if (!first_time) {
            ...
            loadState  = GayLussacLoadState::BRACE_LOADING;
            first_time = true;
        }

        if (loadState == GayLussacLoadState::BRACE_LOADING) {
            pressBrace.onLoad();
            tempBrace.onLoad();
            loadState = GayLussacLoadState::DONE;
        }

        if (press_animating && anim_press_y >= 17) {
            currentPressUnit = targetPressUnit;
            g_unitIsKpa      = currentPressUnit;
            anim_press_y     = 0;
            press_animating  = false;
        }
    ...
    void enterSettings() {
        state = GayLussacState::NEXT_PAGE;
        m_ui.clearFocusManager();

        m_settings.onStartExperiment = [this]() {
            gayLussac.reset();
            gayLussac.setStepMode(g_autoMode ? GayLussacLaw::AUTO : GayLussacLaw::MANUAL);
            gayLussac.setTotalTargetVolume((float)g_initVolume);
            if (g_pressCtrl) gayLussac.setTargetPressure((float)g_targetPressure);
            if (!g_autoMode) {
                gayLussac.clearTempSteps();
                for (int32_t i = 0; i < g_stepCount; i++)
                    gayLussac.addTempStep((float)g_manualSteps[i]);
            }
            gayLussac.start();
            state = GayLussacState::RUNNING;
            m_ui.clearFocusManager();

            m_running.onReport = [this]() {
                state = GayLussacState::REPORT;
                m_ui.clearFocusManager();
        ...
        m_settings.onEnter([this]() {
            state         = GayLussacState::MAIN_IDLE;
            first_time    = false;
            anim_bg       = 0;
            anim_mode_box = 0;
            anim_status_x = 10;
            m_ui.setContinousDraw(true);
            m_ui.addWidgetToFocusManager(&pressBrace);
...
public:
    AppGayLussac(PixelUI& ui, void*) :
        m_ui(ui),
        state(GayLussacState::MAIN_IDLE),
        loadState(GayLussacLoadState::INIT),
        m_settings(ui),
        m_running(ui),
        m_report(ui),
        pressBrace(ui, 3,  45, 56, 18),
        tempBrace (ui, 69, 45, 56, 18),
        nextButton(ui, 89, 33,  8,  7, next_arrow_bits)
    {
        volume = volumeCtrl.getAirVolume();
    }
    ...
    void onEnter(ExitCallback cb) override {
        ...
        pressBrace.setDrawContentFunction([this]() { pressBraceContent(); });
        pressBrace.setCallback([this]() {
            if (press_animating) return;
            targetPressUnit = !currentPressUnit;
            anim_press_y    = 0;
            m_ui.animate(anim_press_y, 18, 300, EasingType::EASE_IN_OUT_CUBIC, PROTECTION::PROTECTED);
            press_animating = true;
        });
    ...
    void draw() override {
        ...
        if (state == GayLussacState::MAIN_IDLE) { drawMainIdle(); return; }
        if (state == GayLussacState::NEXT_PAGE)  { m_settings.draw(); return; }
        if (state == GayLussacState::RUNNING)    { m_running.draw(); return; }
        if (state == GayLussacState::REPORT)     { m_report.draw();  return; }
    }

    bool handleInput(InputEvent event) override {
        if (state == GayLussacState::MAIN_IDLE) {
            if (event == InputEvent::BACK) requestExit();
            return true;
        }
        if (state == GayLussacState::NEXT_PAGE) return m_settings.handleInput(event);
        if (state == GayLussacState::RUNNING)   return m_running.handleInput(event);
        if (state == GayLussacState::REPORT)    return m_report.handleInput(event);
        return true;
    }

    void onExit() override {
        if (state == GayLussacState::RUNNING) gayLussac.stop();
        m_ui.clearAllAnimations();
        m_ui.setContinousDraw(false);
    }
};

AppItem gaylussac_app{
    .title  = "Gay-Lussac Law",
    .bitmap = image_gaylussac_bits,
    .createApp = [](PixelUI& ui, void* parameter) -> std::unique_ptr<IApplication> {
        return std::unique_ptr<IApplication>(new AppGayLussac(ui, parameter));
    },
};
